...
public class AssignmentProblemMAX {
    private static final double FLOATING_POINT_EPSILON = 1E-14;
    private static final int UNMATCHED = -1;

    private int n, m, maxmn;              // number of rows and columns
     ...
     */ 
    public AssignmentProblemMAX(double[][] weight) {
        n = weight.length;
        m = weight[0].length;
        maxmn = weight.length > weight[0].length ? weight.length : weight[0].length;
        this.weight = new double[maxmn][maxmn];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (Double.isNaN(weight[i][j]))
                    throw new IllegalArgumentException("weight " + i + "-" + j  + " is NaN");
                this.weight[i][j] = -weight[i][j];
                if (this.weight[i][j] < minWeight) minWeight = this.weight[i][j];                
            }
        }
        for (int i = 0; i < maxmn; i++) {
            for (int j = 0; j < maxmn; j++) {
            	if (i>= weight.length || j>= weight[0].length) {
            		this.weight[i][j] = Integer.MAX_VALUE;
            	}
            	else this.weight[i][j] -= minWeight;             
            }
        }
    ...
    private void augment() {
        ...
        EdgeWeightedDigraph G = new EdgeWeightedDigraph(2*maxmn+2);
        int s = 2*maxmn, t = 2*maxmn+1;
        for (int i = 0; i < maxmn; i++) {
            if (xy[i] == UNMATCHED)
        ...
        for (int j = 0; j < maxmn; j++) {
            if (yx[j] == UNMATCHED)
                G.addEdge(new DirectedEdge(maxmn+j, t, py[j]));
        }
        for (int i = 0; i < maxmn; i++) {
            for (int j = 0; j < maxmn; j++) {
                if (xy[i] == j) G.addEdge(new DirectedEdge(maxmn+j, i, 0.0));
                else            G.addEdge(new DirectedEdge(i, maxmn+j, reducedCost(i, j)));
            }
        }
    ...
    private void validate(int i) {
        if (i < 0 || i >= maxmn) throw new IndexOutOfBoundsException();
    }
    ...
    private boolean isPerfectMatching() {
        ...
        boolean[] perm = new boolean[maxmn];
        for (int i = 0; i < maxmn; i++) {
            if (perm[xy[i]]) {
    ...
    public static void main(String[] args) {
        ...
        AssignmentProblemMAX assignment = new AssignmentProblemMAX(weight);
