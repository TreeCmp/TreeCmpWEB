...
public class HtmlUtils {
	...
	private String parseShortTable(String data, int firstId, int secondId) {
		...
		table.append(report_table_start);
    ...
    private String parseTable(String data, Boolean isSummary) {
        ...
        if(isSummary == true) {
            lines.remove(0);
            lines.remove(0);
            summarySize = lines.get(1).split(whitespace).length;
        }
        ...
        table.append(line_separator);
        if(isSummary == true) {
            table.append(summary_table_start);
        }
        else {
            table.append(report_table_start);
        }
	...
	private String rowFrom(String data, Boolean header, Boolean isSummary, Boolean shortTable) {
        ...
        if (isSummary == true) {
            ...
            row.append(addClickableRow(header, cells[1], cells[2]));
        }
		...
		/*if(isSummary == false && shortTable == false) {
			row.append(addButton(header, cells[1], cells[2]));
		}
		else {
	...
	private String addClickableRow(Boolean header, String firstId, String secondId) {
		if(header != true) {
			String trees = firstId + "," + secondId;
			return "<tr onclick=\"popup("+ firstId + "," + secondId + ")\" >";
		}
		return tr_start;
	}

/*	private String addButton(Boolean header, String firstId, String secondId) {
		if(header == true) {
...
/*	private String addRadioButton(Boolean header, String firstId, String secondId) {
		if(header == true) {
			return boldedCell("Draw trees with");
		}
		else {
	...
	private final String tfoot_start = "<tfoot>";
    private final String tfoot_end = "</tfoot>";
